...
The bot keeps a healthy, continuously-quoted two-sided market on MWG/USDC (real maker orders that generate genuine volume) **without blowing up inventory or capital** — volume with disciplined risk control is the one thing that must work.
...
- [ ] Resolve MWG/USDC spot market metadata (asset index, tick size, lot/size decimals, min order) from the API
- [ ] Read live L2 order book (best bid/ask, mid) for MWG/USDC
...
- [ ] Track inventory (MWG + USDC) and realized/unrealized PnL
...
- [ ] Run validated on Hyperliquid **testnet** (loop logic on an available pair), then operate on **mainnet** for MWG/USDC
...
- Perpetual futures / leveraged trading — project targets the MWG/USDC **spot** market; no leverage means no liquidation risk
...
- **Platform:** Hyperliquid is a high-performance L1 DEX. Trading is authenticated by signing with an EVM wallet's private key. Spot pairs (e.g. MWG/USDC) use a different asset-naming/indexing scheme than perps and have per-token tick/lot sizes that must be read from the API.
...
- **Strategy reality:** Placing buy below best bid and sell above best ask (2 ticks outside) = passive maker quoting. Volume is generated when real takers fill the orders. Maker orders earn rebates / low fees; an accidental Taker fill costs ~0.035%. The main risk is **inventory risk** — a one-sided fill in a trending market leaves the bot holding MWG (or USDC) at a loss.
...
- **Owner motivation:** MWG is the owner's/team's own token — the bot exists to keep MWG's market liquid and active, with volume + break-even as success, not aggressive profit extraction.
...
|----------|-----------|---------|
| Spot MWG/USDC, not perps | Owner's token is a spot asset; avoid leverage/liquidation risk | — Pending |
...
| Testnet-first, then mainnet MWG | Validate the loop without risking capital before live trading | — Pending |
| Goal = volume near break-even (profit optional) | Support MWG market health; risk discipline over profit chasing | — Pending |
