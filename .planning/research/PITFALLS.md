# Research: Pitfalls — Hyperliquid MWG/USDC Market-Maker

Domain-specific traps, with prevention and the phase that should own each.

| # | Pitfall | Warning signs | Prevention | Phase |
|---|---|---|---|---|
| 1 | **MWG not listed on Hyperliquid** (verified absent on mainnet/testnet) | `spot_meta` has no MWG; orders rejected "unknown asset" | Make target pair fully configurable; validate pair exists on startup and **fail gracefully**; develop/validate on `PURR/USDC`; mainnet MWG gated on HIP-1 listing | Foundation + Cutover |
...
| 4 | **Inventory risk in a trend** | One side keeps filling; growing net MWG or USDC | Inventory caps + **inventory skew** quoting + kill-switch to pause/flatten on one-sided drift | Risk manager |
...
| 11 | **Quoting through thin/illiquid book** (likely for a new token like MWG) | Wide spreads, gaps, your orders are the whole book | Cap size relative to book depth; widen offset in low liquidity; min-spread floor | Quoting/risk |
